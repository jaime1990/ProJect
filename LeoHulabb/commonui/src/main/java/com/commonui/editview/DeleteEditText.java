...
import com.commonutils.KeyBoardUtils;
...
public class DeleteEditText extends EditText implements View.OnFocusChangeListener, TextWatcher
{
    ...
    private void init(Context context)
    {
        ...
        if (mClearDrawable == null) {  
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.M)
                mClearDrawable = context.getResources().getDrawable(R.drawable.icon_close_red);
            else
                mClearDrawable = context.getDrawable(R.drawable.icon_close_red);
        }
    ...
    public void onFocusChange(View v, boolean hasFocus) {  
        ...
        if (hasFocus) {
            setBackgroundResource(R.drawable.focus_delete_edit);
            setClearIconVisible(getText().length() > 0);
        } else {
            setBackgroundResource(R.drawable.normal_delete_edit);
            setClearIconVisible(false);
